...
def interactive_command(args: argparse.Namespace | None) -> None:
    ...
    run_simple_tui(
        ...
        initial_language=getattr(args, "lang", None),
        initial_temperature=getattr(args, "temperature", None),
    )
...
def create_parser() -> argparse.ArgumentParser:
    ...
    common_gen.add_argument("--lang", choices=["fr", "en", "es", "de", "it"], default="fr", help="Langue du template")
    ...
    # Commande 'tui' (principale pour lancer l'interface)
    tui_parser = subparsers.add_parser("tui", parents=[common_gen], help="Lance l'interface TUI")
    tui_parser.add_argument("sujet", nargs="?", default="Wikipédia", help="Sujet initial (défaut: Wikipédia)")
    tui_parser.set_defaults(func=simple_command)

    # Alias rétrocompatible 'simple' (caché dans l'aide)
    simple_parser = subparsers.add_parser("simple", parents=[common_gen], help=argparse.SUPPRESS)
    simple_parser.add_argument("sujet", nargs="?", default="Wikipédia")
...
def main() -> None:
    ...
    # Si un seul argument et que ce n'est pas une sous-commande connue,
    # lancer la TUI avec ce sujet par défaut
    valid_commands = {"generate", "link", "list", "show", "interactive", "tui", "simple"}
    if (
        len(sys.argv) == 2
        and not sys.argv[1].startswith("-")
        and sys.argv[1] not in valid_commands
    ):
        simple_command(argparse.Namespace(
            sujet=sys.argv[1],
            model=None,
            provider=None,
            temperature=0.7,
