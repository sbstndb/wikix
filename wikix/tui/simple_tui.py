...
class SimpleTUI:
    ...
    def _draw_history_panel(self, stdscr, height, separator_line, left_panel_width):
        ...
        if panel_start_line >= 0 and left_panel_width > 2:
            try:
                ...
                try:
                    ...
                    pass  # Terminal trop petit

        # Utiliser l'ordre d'insertion (stable) au lieu du tri par timestamps
        ordered_history = list(self.state.full_history)
        ...
        def build_tree_recursive(node: str, prefix: str = "", is_last: bool = True) -> list[str]:
            ...
            children = [s for s in ordered_history if self.state.subject_parent.get(s) == node]
        ...
        # Trouver toutes les racines (sujets sans parent) dans l'ordre d'insertion
        roots = [s for s in ordered_history if self.state.subject_parent.get(s) is None]
        ...
        # Si pas d'arbre, utiliser la liste simple telle quelle (ordre d'insertion)
        if not tree_lines:
            tree_lines = list(ordered_history)
        ...
        if not self.state.history_mode:
            navigation_list = tree_lines if tree_lines else ordered_history
        ...
        for idx, subj in enumerate(items_slice):
            ...
            if line_no >= 0 and left_panel_width > 2:
                try:
                    if is_selected:
    ...
    def force_fallback_provider(self):
        """Force un changement vers un provider différent pour récupérer d'une erreur."""
        # Ordre de fallback préféré (évite le provider actuel)
        fallback_order = ["openai", "openrouter", "cerebras", "gemini"]
        
        # Retirer le provider actuel de la liste
        if self.state.current_provider in fallback_order:
            fallback_order.remove(self.state.current_provider)
        
        # Prendre le premier provider disponible
        if fallback_order:
            self.state.current_provider = fallback_order[0]
            self._reconcile_model_with_provider()
            self.state.force_redraw = True

    def _sync_generating_content(self, subject: str):
        """Synchronise le contenu affiché avec un sujet en cours de génération."""
        if subject in self.state.generating_subjects and subject in self.state.subject_texts:
            with self.state.text_lock:
                # Récupérer le contenu le plus récent du cache
                latest_content = self.state.subject_texts.get(subject, "")
                # Si on a du contenu réel (pas le placeholder), l'afficher
                if latest_content and not latest_content.startswith(GENERATING_MSG_PREFIX):
                    self.state.current_text = latest_content
                    self.state.force_redraw = True
                # Sinon, si on est vraiment en cours de génération mais sans contenu encore,
                # afficher le placeholder de génération
                elif subject in self.state.generating_subjects and (not latest_content or latest_content.startswith(GENERATING_MSG_PREFIX)):
    ...
    def start_input_mode(self):
        ...
        elif self.state.current_language == "de":
            self.state.input_prompt = "Geben Sie ein Thema ein:"
        elif self.state.current_language == "it":
            self.state.input_prompt = "Inserisci un argomento:"
        else:
    ...
    def generate_fiche_streaming_thread(self, subject: str, previous_text: str):
        ...
        try:
            ...
            with self.state.text_lock:
                # Mettre à jour le cache avec le message d'erreur
                self.state.subject_texts[subject] = error_message
                # Afficher l'erreur seulement si on est toujours sur ce sujet
                if self.state.current_subject == subject:
                    self.state.current_text = error_message
                    self.state.force_redraw = True
            self.state.is_generating = False
    ...
    def _format_error_message(self, error: Exception) -> str:
        ...
        if "quota" in error_str or "billing" in error_str:
            return (f"❌ **Quota API épuisé**\n\n"
                   f"Le quota de votre API a été dépassé. Solutions :\n\n"
                   f"1. **Changez de provider** : Appuyez sur 'p' pour changer de fournisseur\n"
                   f"2. **Changez de modèle** : Appuyez sur 'm' pour un modèle différent\n"
                   f"3. **Vérifiez vos quotas** dans les consoles des APIs\n"
                   f"4. **Redémarrez** l'application pour tenter un fallback automatique\n\n"
                   f"**Détail technique :** {str(error)}")
        
        elif "connection" in error_str or "network" in error_str or "timeout" in error_str:
            return (f"🌐 **Problème de connexion**\n\n"
                   f"Impossible de se connecter au service. Solutions :\n\n"
                   f"1. **Vérifiez votre connexion Internet**\n"
                   f"2. **Réessayez** avec 'r' (regenerate)\n"
                   f"3. **Changez de provider** avec 'p'\n\n"
                   f"**Détail :** {str(error)}")
        
        elif "api" in error_str and ("key" in error_str or "auth" in error_str):
            return (f"🔑 **Problème d'authentification**\n\n"
                   f"Votre clé d'API semble incorrecte ou manquante. Solutions :\n\n"
                   f"1. **Vérifiez vos variables d'environnement**\n"
                   f"   - OPENAI_API_KEY\n"
                   f"   - GEMINI_API_KEY\n"
                   f"   - CEREBRAS_API_KEY\n"
                   f"   - OPENROUTER_API_KEY\n"
                   f"2. **Changez de provider** avec 'p'\n\n"
                   f"**Détail :** {str(error)}")
        
        else:
            return (f"⚠️ **Erreur de génération**\n\n"
                   f"Une erreur inattendue s'est produite. Solutions :\n\n"
                   f"1. **Réessayez** avec 'r' (regenerate)\n"
                   f"2. **Changez de provider** avec 'p' ou de modèle avec 'm'\n"
                   f"3. **Redémarrez** l'application si le problème persiste\n\n"
                   f"**Détail :** {str(error)}")
    ...
    def draw_screen(self, stdscr):
        ...
        if self.state.history_mode:
            ...
            instructions = f"🔄📝 Generating + Selection | 't': {theme_name} | 'l': {lang_info['flag']} | 'm': {model_info['name']} | 'p': {provider_info} | '+/-': Temp {self.state.current_temperature:.2f} | 's': Stop | Tab/Shift+Tab: History"
        elif self.state.is_generating:
            instructions = f"🔄 Generating | Arrows: Navigate | 't': {theme_name} | 'l': {lang_info['flag']} | 'm': {model_info['name']} | 'p': {provider_info} | '+/-': Temp {self.state.current_temperature:.2f} | 's': Stop | Tab/Shift+Tab: History"
        elif self.state.generating_subjects and self.state.selection_mode:
            instructions = f"🔄📝 Background Gen + Selection | 't': {theme_name} | 'l': {lang_info['flag']} | 'm': {model_info['name']} | 'p': {provider_info} | '+/-': Temp {self.state.current_temperature:.2f} | 's': Stop | Esc: Cancel | Tab/Shift+Tab: History"
        elif self.state.generating_subjects:
            instructions = f"🔄 Background Generating | Arrows: Navigate | 't': {theme_name} | 'l': {lang_info['flag']} | 'm': {model_info['name']} | 'p': {provider_info} | '+/-': Temp {self.state.current_temperature:.2f} | 's': Stop | Tab/Shift+Tab: History"
        elif self.state.selection_mode:
            instructions = f"📝 Selection mode | Arrows: Expand | Enter: Generate | 't': {theme_name} | 'l': {lang_info['flag']} | 'm': {model_info['name']} | 'p': {provider_info} | '+/-': Temp {self.state.current_temperature:.2f} | 's': Stop | Esc: Cancel | Tab/Shift+Tab: History"
        else:
            hist_label = "Show" if not self.state.history_visible else "Hide"
            instructions = f"Arrows: Navigate | Enter: Generate | Space: Select | 'r': Regenerate | 'i': Input | 't': {theme_name} | 'l': {lang_info['flag']} | 'm': {model_info['name']} | 'p': {provider_info} | '+/-': Temp {self.state.current_temperature:.2f} | 's': Stop | 'h': {hist_label} History | Tab/Shift+Tab: History | 'b': Back | 'f': Force fallback | 'q': Quit"
        ...
        if separator_line < height and width > 0:
            try:
                ...
                try:
                    ...
                    pass  # Ignore si le terminal est trop petit
        ...
        if self.state.words:
            ...
            temp_indicator = f"🌡️ {self.state.current_temperature:.2f}"
            status_parts = [status, theme_indicator, lang_indicator, model_indicator, provider_indicator, temp_indicator]
    ...
    def handle_input(self, stdscr, key):
            ...
            if key in (ord("\n"), ord("\r")):
                ...
                if 0 <= self.state.history_cursor < len(lines):
                    ...
                    if selected_subject in self.state.subject_texts and self.state.subject_texts[selected_subject]:
                        ...
                        # Si on revient sur un sujet en cours de génération, synchroniser le contenu
                        if self.state.is_generating:
                            self._sync_generating_content(selected_subject)
                    else:
        ...
        if self.state.input_mode:
            ...
            if result:
                self.state.input_mode = False
                # Conserver un historique stable: ne pas réordonner full_history ici
        ...
        if key == ord("q"):
            ...
            # Force fallback - change vers un provider différent et régénère
            self.force_fallback_provider()
            if not self.state.is_generating:
                # Regénérer automatiquement avec le nouveau provider
                self.load_subject_streaming(self.state.current_subject, stdscr)
        elif key in (ord("+"), ord("=")):
            # Augmenter température (borne à 1.0)
            self.state.current_temperature = min(1.0, self.state.current_temperature + 0.05)
            self.state.force_redraw = True
        elif key == ord("-"):
            # Diminuer température (borne à 0.0)
            self.state.current_temperature = max(0.0, self.state.current_temperature - 0.05)
            self.state.force_redraw = True
        elif key == ord("i"):
            ...
            # Si on revient sur un sujet en cours de génération, synchroniser le contenu
            if self.state.is_generating:
                self._sync_generating_content(prev_subject)
        ...
        # Global Tab: cycle through history in visual order (tree order).
        elif key == ord('\t'):
            ...
            if navigation_list:
                ...
                if next_subject:
                    if next_subject in self.state.subject_texts and self.state.subject_texts.get(next_subject):
                        # Afficher le cache existant
                        ...
                        # Si on revient sur un sujet en cours de génération, synchroniser le contenu
                        if self.state.is_generating:
                            self._sync_generating_content(next_subject)
                    else:
                        # Démarrer la génération si le sujet n'a pas encore de contenu
                        self.load_subject_streaming(next_subject, stdscr, parent=self.state.current_subject)
                        # Mettre à jour le curseur d'historique
                        self.state.history_cursor = next_index
            return False
        # Global Shift-Tab: cycle backward through history in visual order.
    ...
    def load_subject_streaming(self, subject: str, stdscr, parent: str | None = None):
        ...
        if subject not in self.state.full_history:
            # Ajouter à la fin pour préserver l'ordre d'insertion stable
            self.state.full_history.append(subject)
            self.state.subject_timestamps[subject] = time.time()
        ...
        # Lancer la génération dans un thread avec gestion d'erreurs améliorée
        thread = threading.Thread(
            target=self._safe_generate_fiche_streaming_thread,
            args=(subject, previous_text),
            daemon=True
        )
        thread.start()

    def _safe_generate_fiche_streaming_thread(self, subject: str, previous_text: str):
        """Wrapper sécurisé pour la génération en streaming avec gestion d'erreurs complète."""
        try:
            self.generate_fiche_streaming_thread(subject, previous_text)
        except Exception as e:
            # Capture toute exception non gérée dans le thread
            error_message = self._format_error_message(e)
            with self.state.text_lock:
                self.state.subject_texts[subject] = error_message
                if self.state.current_subject == subject:
...
def run_simple_tui(
    ...
    *,
    ...
    """Lance l'interface TUI simple avec options initiales."""
    tui = SimpleTUI(initial_subject)
    # Appliquer les options initiales si fournies
    if initial_model is not None:
        tui.state.current_model = initial_model
    if initial_provider is not None:
        tui.state.current_provider = initial_provider
        # Réconcilier modèle/provider
        tui.cycle_provider()  # force reconciliation path, then set back
        tui.state.current_provider = initial_provider
    if initial_language is not None:
        if initial_language in tui.state.languages:
            tui.state.current_language = initial_language
    if initial_temperature is not None:
        try:
            temp = float(initial_temperature)
            tui.state.current_temperature = max(0.0, min(1.0, temp))
        except Exception:
            pass
